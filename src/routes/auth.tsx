  ...
  return (
    <div className="mx-auto max-w-md px-4 py-20">
      ...
      <Button
        type="button"
        variant="outline"
        className="w-full h-12 text-xs uppercase tracking-widest gap-3"
        onClick={async () => {
          const result = await lovable.auth.signInWithOAuth("google", {
            redirect_uri: window.location.origin,
          });
          if (result.error) toast.error(result.error.message || "Google sign-in failed");
          else if (!result.redirected) navigate({ to: search.next ?? "/account/orders" });
        }}
      >
      ...
      <div className="relative my-6">
        ...
        <div className="relative flex justify-center text-xs uppercase tracking-widest"><span className="bg-background px-3 text-muted-foreground">Or with email</span></div>
      </div>
